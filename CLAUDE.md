...
- [2026-06] 任务存储用内存字典，前提是 Cloud Run 配置为**单实例**；如果未来开启多实例自动扩容，必须先切换到 Redis 或等价的跨实例存储，否则会出现同一批任务在不同实例间状态不一致（不报错但结果错误的隐蔽 bug）
- [2026-06-17] smoke-test.sh 只检查任务提交状态码（202）不够——"创建→查询"异步链路必须显式验证：提交后解析响应体的 task_id，用真实 id 去查状态路由，两步分开 PASS 不等于链路连通
- [2026-06-17] extract_json_field() 用 grep -o 在无匹配时返回非零退出码，在 set -e 脚本里会让整个脚本提前静默退出 → 对"预期内可能无结果"的命令必须显式加 `|| true`
- [2026-06-17] smoke-test.sh 不应依赖 jq 等外部二进制——安全绳本身不能比被测系统更脆弱，Cloud Run 基础镜像不保证装了 jq → 用纯 grep/sed 实现最小化 JSON 字段提取
