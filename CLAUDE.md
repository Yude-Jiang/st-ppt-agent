...
- 项目已知坑（追加段落，插入 CLAUDE.md 的「项目已知坑」标题下）


[2026-06-17] smoke-test.sh 早期版本只检查任务提交的 HTTP 状态码（202），没有解析响应
body 里的 task_id 字段去验证后续状态查询路由——这意味着"创建任务→查询任务"这条
异步链路实际上没有被端到端验证过，哪怕所有 check 都显示 PASS。修复：提交后用
extract_json_field() 从响应体解析出真实 task_id，再用这个 id 去查状态路由。
→ 通用教训：任何"创建→用创建结果做下一步"的异步流程，写测试时默认假设这条链路
是断的，除非显式验证过用第一步的真实输出去驱动第二步。仅检查每一步自己的状态码，
两步分开看都"通过"，并不能证明链路是连起来的。
[2026-06-17] smoke-test.sh 里自定义的 extract_json_field()（用 grep -o 从 JSON 提取
字段，不依赖 jq）在找不到匹配字段时，grep 返回非零退出码；在 set -e 生效的脚本里，
这个非零退出码会让整个脚本在该函数调用处提前退出，后面所有检查（包括负面用例）都
没机会跑，但脚本表现得像是"正常执行到这里"，不会用明显的报错提示你这一点。
→ 修复：对"正常情况下也可能无匹配/无结果"的命令，显式加 || true，不让 set -e
误伤这类预期内的空结果场景。
→ 通用教训：在 set -euo pipefail 的脚本里，任何"我知道这步可能拿不到结果，拿不到
就该走 FAIL 分支而不是让脚本死掉"的命令，必须显式加 || true 或等价处理，否则
set -e 会让脚本在错误最该被看见的地方反而悄悄退出。
[2026-06-17] smoke-test.sh 不应该依赖 jq 这类外部二进制——它的角色是部署后的"安全绳"，
本身应该比被测系统更不容易因环境缺失而失败。第一次在本地容器里装 jq 时遇到镜像源
瞬时失败，提醒了这一点：不能假设目标部署环境（Cloud Run 基础镜像）一定装了 jq。
→ 修复：用纯 grep/sed 实现最小化 JSON 字段提取，不引入额外依赖。
